...
| cdm\_certificates\_bucket | The name of the S3 bucket where the CDM certificate files live. | `string` | n/a | yes |
| cdm\_certificates\_objects | The CDM certificate objects inside the bucket. | `list(string)` | ```[ "cert_US_CISA_0_Root_CA.crt", "cert_US_CISA_1_Issuing_CA_reissued.crt" ]``` | no |
| tags | Tags to apply to all AWS resources created | `map(string)` | ```{ "Application": "ansible-role-cdm-certificates testing", "Team": "VM Fusion - Development" }``` | no |
...
| user | The IAM user being created to test the cisagov/ansible-role-cdm-certificates Ansible role. |
| user\_access\_key | The IAM access for the IAM user being created to test the cisagov/ansible-role-cdm-certificates Ansible role. |
<!-- END_TF_DOCS -->
